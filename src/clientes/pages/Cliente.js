...
export const Cliente = ({ setHeader }) => {
  ...
  useEffect(() => {
    console.log("render");
    axios
      .get(`http://localhost:4000/api/db/clientes/${params.id}`)
      .then((response) => {
    ...
    axios
      .get(`http://localhost:4000/api/db/clientes/${params.id}/mails`)
      .then((response) => {
    ...
    axios
      .get(`http://localhost:4000/api/db/clientes/${params.id}/telefonos`)
      .then((response) => {
    ...
    axios
      .get(
        `http://localhost:4000/api/db/clientes/${params.id}/forma-de-facturacion`
      )
      .then((response) => {
    ...
    axios
      .get(`http://localhost:4000/api/db/clientes/${params.id}/forma-de-pago`)
      .then((response) => {
    ...
    axios
      .get(`http://localhost:4000/api/db/clientes/${params.id}/observaciones`)
      .then((response) => {
    ...
    axios
      .get(`http://localhost:4000/api/db/clientes/${params.id}/embarcaciones`)
      .then((response) => {
        setListaEmb(JSON.parse(JSON.stringify(response.data)));
      });

    axios
      .get("http://localhost:4000" + "/api/db/formas-de-pago")
      .then((response) => {
  ...
  let handleGuardarDetalleFP = (formaGuardada) => {
    ...
    setFormaPago((prevFormaPago) => {
      let newFormaPago = prevFormaPago.map((forma) => {
        if (
          forma.idforma_de_pago_has_clientes ===
          formaGuardada.idforma_de_pago_has_clientes
        ) {
  ...
  let guardarCambios = () => {
    ...
    //comparar solo el ID
    cambios.forma_de_facturacion.eliminar = objetosNoIncluidosID(
      snapFormaFacturacion,
      ffModificarEliminar,
      "idforma_de_facturacion"
    );
    ...
    //formas de pago
    cambios.forma_de_pago.insertar = objetosNoIncluidos(
      formaPago,
      snapFormaPago
    );
    cambios.forma_de_pago.eliminar = objetosNoIncluidos(
      snapFormaPago,
      formaPago
    );
    ...
    //observaciones
    cambios.observaciones.insertar = objetosNoIncluidos(
      observaciones,
      snapObservaciones
    );
    cambios.observaciones.eliminar = objetosNoIncluidos(
      snapObservaciones,
      observaciones
    );
    ...
    cambios.telefonos.insertar.length === 0 &&
      delete cambios.telefonos.insertar;
    cambios.telefonos.eliminar.length === 0 &&
      delete cambios.telefonos.eliminar;
    Object.keys(cambios.telefonos).length === 0 && delete cambios.telefonos;
    cambios.forma_de_pago.insertar.length === 0 &&
      delete cambios.forma_de_pago.insertar;
    cambios.forma_de_pago.eliminar.length === 0 &&
      delete cambios.forma_de_pago.eliminar;
    Object.keys(cambios.forma_de_pago).length === 0 &&
      delete cambios.forma_de_pago;
    cambios.observaciones.insertar.length === 0 &&
      delete cambios.observaciones.insertar;
    cambios.observaciones.eliminar.length === 0 &&
      delete cambios.observaciones.eliminar;
    Object.keys(cambios.observaciones).length === 0 &&
      delete cambios.observaciones;
    ...
    cambios.forma_de_facturacion.eliminar.length === 0 &&
      delete cambios.forma_de_facturacion.eliminar;
    cambios.forma_de_facturacion.insertar.length === 0 &&
      delete cambios.forma_de_facturacion.insertar;
    cambios.forma_de_facturacion.modificar.length === 0 &&
      delete cambios.forma_de_facturacion.modificar;
    Object.keys(cambios.forma_de_facturacion).length === 0 &&
      delete cambios.forma_de_facturacion;

    axios
      .put(
        `http://localhost:4000/api/db/clientes/${params.id}/guardar-cambios`,
        cambios
      )
      .then((response) => {
  ...
  let handleChangeFormaPago = (obj) => {
    let objCompleto = {
      ...obj,
      idforma_de_pago_has_clientes: Math.random(),
      numero: null,
      clientes_idclientes: Number(params.id),
    };
    setFormaPago((prevFormaPago) => {
  ...
  let validarFormaPago = (valor, callback) => {
    let forma = listaFormasDePago.current.filter(
      (forma) => forma.descripcion === valor
    );
    if (forma.length > 0) {
      callback({
        descripcion: valor,
        forma_de_pago_idforma_de_pago: forma[0].idforma_de_pago,
      });
    }
  };
  ...
  return (
    <div className="clientes__container">
      <div className="cliente__datos-cliente">
        {cliente
          ? llavesCliente.map((llave) => {
              let tipo =
                llave === "fecha_registro" || llave === "fecha_ingreso"
                  ? "date"
                  : "text";
              let label =
                llave === "idclientes" ? "ID" : llave.replace(/_/g, " ");
      ...
      <div
        className={`cliente__flag ${
          modificado ? "cliente__flag--visible" : ""
        }`}
      >
        {modificado ? (
          <FlagModificado
