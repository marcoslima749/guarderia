...
export const Resumen = ({
  setHeader,
  /* cuentaCorriente,  listaResumen,*/ clases,
}) => {
  ...
  useEffect(() => {
    ...
    setHeader.setPanelHeader(
      <Boton path="#" clases="simple-hover embarcacion__boton-nuevo">
        Nuevo
      </Boton>
    );
  }, []);
  ...
  useEffect(() => {
    axios.get("http://localhost:4000/api/db/resumen").then((response) => {
      console.log(response.data);
      actualizarLista(response.data);
      const arrConsultas = response.data.map((el) =>
        axios.get(`http://localhost:4000/api/db/clientes/${el.IDc}/cta-cte`)
      );
  ...
  let titulos = llaves
    .filter((llave) => llave !== "IDc")
    .map((llave) => {
      return (
        <span
          key={Math.random()}
          className={`simple-hover embarcaciones__titulo embarcaciones__titulo-${llave}`}
        >
          {llave}
        </span>
      );
  ...
  let filas = listaEmb.map((emb) => {
    return (
      <div
        key={`${emb.ID}-${emb.Cliente}`}
        className="embarcaciones__embarcacion"
      >
        {llaves
          .filter((llave) => llave !== "IDc")
          .map((llave) => {
              ...
              <span
                ...
                className={`${
                  ...
                  llave === "Estado"
                    ? "simple-hover"
                    : ""
                } embarcaciones__campo embarcacion__${llave}`}
              >
                {llave === "Embarcacion" ? (
                  <Boton path={`/embarcaciones/${emb.ID}`}>{emb[llave]}</Boton>
                ) : llave === "Cliente" ? (
                  <Boton path={`/clientes/${emb.IDc}`}>{emb[llave]}</Boton>
                ) : llave === "Estado" ? (
  ...
  return (
    <div className={`embarcaciones ${clases}`}>
      <div className="embarcaciones__titulos">{titulos}</div>

      <div className="embarcaciones__lista">{filas}</div>
      <div className="embarcaciones__flag">flags</div>
    </div>
  );
};
