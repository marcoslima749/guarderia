...
export const Embarcacion = ({ setHeader }) => {
  ...
  useEffect(() => {
    ...
    /*
        ...
        */
    axios
      .get(`http://localhost:4000/api/db/embarcaciones/${params.id}`)
      .then((response) => {
  ...
  let guardarCambios = () => {
    ...
    if (campos.length === 0) {
      ...
      if (
        ...
        embarcacion.contrato = moment(embarcacion.contrato).isValid()
          ? moment(embarcacion.contrato).format("YYYY[-]MM[-]DD")
          : null;
        embarcacion.seguro = moment(embarcacion.seguro).isValid()
          ? moment(embarcacion.seguro).format("YYYY[-]MM[-]DD")
          : null;
        console.log(
          "contrato: ",
          embarcacion.contrato,
          "seguro: ",
          embarcacion.seguro
        );
      ...
      axios
        .put(`http://localhost:4000/api/db/embarcaciones/${embarcacion.Id}/m`, {
          embarcacion,
          campos,
        })
        .then((response) => {
  ...
  return (
    <div className="embarcacion__container">
      ...
      <div
        className={`embarcacion__flag ${
          modificado ? "embarcacion__flag--visible" : ""
        }`}
      >
        {modificado ? (
          <FlagModificado
