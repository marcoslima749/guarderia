...
export const Tareas = ({ setHeader }) => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:4000/api/db/tareas")
      .then((res) => {
        setLista(() =>
          res.data.map((tarea) => {
            let tareaMod = {
              ...tarea /*, modificado : false, eliminado: false, nuevo: false*/,
            };
            return tareaMod;
          })
        );
  ...
  const eliminarEnLista = (tareaEliminada) => {
    setLista((prevLista) => {
      let newLista = prevLista.filter(
        (tarea) => tarea.idtareas !== tareaEliminada.idtareas
      );
      return newLista;
    });
  };
  ...
  const enviarConsulta = (tarea, mod, campos = []) => {
    ...
    axios
      .post("http://localhost:4000/api/db/tareas", {
        tarea,
        mod,
        campos,
      })
      .then((response) => {
        ...
        if (mod === "nuevo") {
          console.log(
            "insertada exitosamente una tarea nueva. Cambiando id en la lista"
          );
          setLista((prevLista) => {
            let newLista = prevLista.map((tareaEnLista) => {
              if (tareaEnLista.idtareas === tarea.idtareas) {
                console.log(
                  "coinciden las id viejas. Id en la lista: " +
                    tareaEnLista.idtareas +
                    " id en la tarea mandada: " +
                    tarea.idtareas
                );
                console.log("response: ", response);
          ...
          console.log("mod no es nuevo: ", mod);
        }
  ...
  const handleFocus = (indice, id) => {
    ...
    if (elementoAnterior === undefined) {
      //Es el primer elemento en hacer focus
      console.log("elemento anterior undefined: " + elementoAnterior);
      console.log(
        "NOOO se va a setear el anterior a la lista[indice]: " +
          JSON.stringify(lista[indice])
      );
      console.log(
        "se va a setear el anterior a AL ID ENFOCADO " + JSON.stringify(foco)
      );
      ...
      //El focus está dentro del mismo elemento
      return;
    }
  ...
  const verificarCambios = () => {
    ...
    if (camposModificados.length === 0) {
      ...
      console.log(
        "los campos son iguales, no hay modificacion. lastTarea: ",
        lastTarea,
        " snap: ",
        snap
      );
  ...
  const handleBlur = (indice, id) => {
    ...
    if (elementoAnterior.idtareas === id) {
      console.log(
        "el elemento anterior es igual al actual (posible ultimo blur)"
      );
      //checkear que no hay focus en el resto de los elementos para determinar que es el último
      setTimeout(() => {
        let ultimoBlur = ![].slice
          .call(document.getElementsByClassName("lista-tareas__tarea"))
          .some((el) => el.matches(":focus-within"));
        if (ultimoBlur) {
    ...
    <div className="lista-tareas__container">
      <ul className="lista-tareas">
        <li className="lista-tareas__tarea lista-tareas__header">
          ...
          <div
            onClick={(e) => handleDesplazar(e)}
            className="lista-tareas__agregar simple-hover"
          >
        ...
        {lista &&
          lista
            .filter(
              (tarea) =>
                tarea.completado ===
                (mostrarCompletadas && completadas > 0 ? 1 : 0)
            )
            .map((tarea, indice) => {
              return (
                <li
                  onFocus={() => handleFocus(indice, tarea.idtareas)}
                  onBlur={() => handleBlur(indice, tarea.idtareas)}
                  key={tarea.idtareas}
                  className={`lista-tareas__tarea simple-hover ${
                    transition && "lista-tareas__tarea--transition"
                  } ${desplazar && "lista-tareas__desplazar"}`}
                >
                  <div
                    className="lista-tareas__completado"
                    onClick={(e) => completarTarea(indice, tarea.idtareas, e)}
                  >
                  ...
                  <div className="lista-tareas__info">
                    <div className="lista-tareas__campo-descripcion">
                      <input
                        type="text"
                        onChange={(e) => handleChange(e, tarea.idtareas)}
                        name="descripcion"
                        spellCheck="false"
                        className="lista-tareas__descripcion lista-tareas__campo"
                    ...
                    <div className="lista-tareas__campo-nota">
                      <input
                        ...
                        name="nota"
                        spellCheck="false"
                        className="lista-tareas__nota lista-tareas__campo"
                        value={tarea.nota ? tarea.nota : ""}
                      />
                    </div>
                  </div>
                  <input
                    ...
                    name="deadline"
                    onChange={(e) => handleChange(e, tarea.idtareas)}
                    className="lista-tareas__deadline lista-tareas__campo"
                    value={
                      tarea.deadline
                        ? moment(tarea.deadline).format("YYYY[-]MM[-]DD")
                        : ""
                    }
                  />
                  <div
                    className="lista-tareas__eliminar"
                    onClick={() => eliminarTarea(tarea.idtareas)}
                  >
                    <img
                      src={eliminar}
                      className="lista-tareas__eliminar__icono"
                      alt="eliminar"
                    />
                  </div>
                  <div className="lista-tareas__campo-prioridad">
                    <input
                      ...
                      name="prioridad"
                      spellCheck="false"
                      className="lista-tareas__prioridad lista-tareas__campo"
      ...
      <div
        className={`tareas-completadas__container simple-hover ${
          completadas > 0 && "tareas-completadas__container--visible"
        }`}
      >
        <span
          onClick={() => {
            setMostrarCompletadas((mc) => !mc);
          }}
          className="tareas-completadas__span"
        >
          {mostrarCompletadas
            ? `Ocultar tareas completadas (${completadas})`
            : `Mostrar tareas completadas (${completadas})`}
        </span>
      </div>
    </div>
  );
